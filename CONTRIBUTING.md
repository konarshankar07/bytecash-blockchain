...
Welcome to the bytecash-blockchain project!
We are happy that you are taking a look at the code for Bytecash, a proof of space and time cryptocurrency.

A lot of fascinating new cryptography and blockchain concepts are used and implemented here.
This repo includes the code for the Bytecash full node, farmer, and timelord (in bytecash folder), which are all written in python.
It also includes a verifiable delay function implementation that it imports from the [chiavdf repo](https://github.com/Bytecash-Network/chiavdf) (in c/c++), and a proof of space implementation that it imports from the [chiapos repo](https://github.com/Bytecash-Network/chiapos). BLS signatures are imported from the [bls-signatures repo](https://github.com/Bytecash-Network/bls-signatures) as blspy. There is an additional dependency on the [chiabip158 repo](https://github.com/Bytecash-Network/chiabip158). For major platforms, binary and source wheels are shipped to PyPI from each dependent repo. Then bytecash-blockchain can pip install those from PyPI or they can be prepackaged as is done for the Windows installer. On unsupported platforms, pip will fall back to the source distributions, to be compiled locally.

If you want to learn more about this project, read the [wiki](https://github.com/Bytecash-Network/bytecash-blockchain/wiki), or check out the [green paper](https://www.bytecash.net/assets/ChiaGreenPaper.pdf).
...
Contact any of the team members on [Keybase](https://keybase.io/team/bytecash_network.public), which we use as the main communication method. You can also comment on any Trello card.

We ask that external contributors create a fork of the `main` branch for any feature work they wish to take on.

Members of the Bytecash organization may create feature branches from the `main` branch.

In the event an emergency fix is required for the release version of Bytecash, members of the Bytecash organization will create a feature branch from the current release branch `1.0.0`.
...
4. Bytecash Network's nodes on the public testnet are running the latest version `x.y.z`.
...
. ./activate
pip install ".[dev]"
black bytecash tests && mypy bytecash tests && flake8 bytecash tests
...
We provide a [pre-commit configuration](https://github.com/Bytecash-Network/bytecash-blockchain/blob/main/.pre-commit-config.yaml) which triggers several useful
...
7. Preferences > Settings > mypy > Targets: set to `./bytecash` and `./tests`
...
6. Install these linters https://github.com/Bytecash-Network/bytecash-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the bytecash blockchain.
...
Bytecash organization members have been granted CI access to deploy `beta testnets`.
If you are not a Bytecash organization member, you can enquire about deploying a `beta testnet` in the public dev Keybase channel.

## Submit changes

To propose changes, please make a pull request to the `main` branch. See Branching Strategy above.

To propose changes for the production releases of Bytecash, please make a pull request to the latest release branch.
