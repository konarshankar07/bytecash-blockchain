...
from bytecash.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from bytecash.full_node.bundle_tools import simple_solution_generator
...
from bytecash.types.announcement import Announcement
...
from bytecash.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from bytecash.wallet.puzzles.puzzle_utils import (
...
from bytecash.wallet.secret_key_store import SecretKeyStore
from bytecash.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_chia(self, bytecash_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if bytecash_amount < 0:
            utxos = await self.select_coins(abs(bytecash_amount), exclude)
        else:
        ...
        bytecash_amount = spend_value + bytecash_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": bytecash_amount}]
