...
from bytecash.wallet.cc_wallet import cc_utils
from bytecash.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from bytecash.wallet.cc_wallet.cc_wallet import CCWallet
from bytecash.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from bytecash.wallet.trade_record import TradeRecord
from bytecash.wallet.trading.trade_status import TradeStatus
from bytecash.wallet.trading.trade_store import TradeStore
from bytecash.wallet.transaction_record import TransactionRecord
from bytecash.wallet.util.trade_utils import (
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_spends:
            ...
            if r:
                ...
                # standard bytecash coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if bytecash_discrepancy is None:
                    bytecash_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    bytecash_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        bytecash_spend_bundle: Optional[SpendBundle] = None
        if bytecash_discrepancy is not None:
            bytecash_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_chia(
                bytecash_discrepancy, []
            )
            if bytecash_spend_bundle is not None:
                for coinsol in coinsols:
                    bytecash_spend_bundle.coin_spends.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if bytecash_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = bytecash_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have bytecash available",
                        )
        ...
        now = uint64(int(time.time()))
        if bytecash_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, bytecash_spend_bundle])
            if bytecash_discrepancy < 0:
                # TODO: address hint error and remove ignore
                ...
                tx_record = TransactionRecord(
                    ...
                    amount=uint64(abs(bytecash_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=bytecash_spend_bundle,
                    additions=bytecash_spend_bundle.additions(),
                    removals=bytecash_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=bytecash_spend_bundle.name(),
                )
            else:
                # TODO: address hint error and remove ignore
                #       error: Argument "to_puzzle_hash" to "TransactionRecord" has incompatible type "bytes"; expected
