...
import stat
import sys
from bytecash.util.config import load_config, traverse_dict
from bytecash.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `bytecash init --fix-ssl-permissions` to fix issues.")
