...
from bytecash.cmds.configure import configure_cmd
from bytecash.cmds.farm import farm_cmd
from bytecash.cmds.init import init_cmd
from bytecash.cmds.keys import keys_cmd
from bytecash.cmds.netspace import netspace_cmd
from bytecash.cmds.passphrase import passphrase_cmd
from bytecash.cmds.plots import plots_cmd
from bytecash.cmds.show import show_cmd
from bytecash.cmds.start import start_cmd
from bytecash.cmds.stop import stop_cmd
from bytecash.cmds.wallet import wallet_cmd
from bytecash.cmds.plotnft import plotnft_cmd
from bytecash.cmds.plotters import plotters_cmd
from bytecash.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from bytecash.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage bytecash blockchain infrastructure ({__version__})\n",
    epilog="Try 'bytecash start node', 'bytecash netspace -d 192', or 'bytecash show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from bytecash.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from bytecash.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show bytecash version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs bytecash daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by bytecash start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from bytecash.daemon.server import async_run_daemon
