...
import bytecash.cmds.configure as bytecash_configure
from bytecash import __version__
from bytecash.cmds.bytecash import monkey_patch_click
from bytecash.cmds.init_funcs import init
from bytecash.seeder.util.config import patch_default_seeder_config
from bytecash.seeder.util.service_groups import all_groups, services_for_groups
from bytecash.seeder.util.service import launch_service, kill_service
...
@click.group(
    help=f"\n  Manage the Bytecash Seeder ({__version__})\n",
    epilog="Try 'bytecash seeder start crawler' or 'bytecash seeder start server'",
...
@cli.command("version", short_help="Show the Bytecash Seeder version")
...
def init_cmd(ctx: click.Context, **kwargs):
    print("Calling Bytecash Seeder Init...")
    ...
    if root_path.is_dir() and not Path(root_path / "config" / "config.yaml").exists():
        ...
        # This really shouldn't happen, but if we dont have the base bytecash config, we can't continue
...
def configure(
    ...
    bytecash_configure.configure(root_path, "", "", "", "", "", "", "", "", testnet, "")

    config: Dict = load_config(DEFAULT_ROOT_PATH, "config.yaml")
    change_made = False
    if testnet is not None:
        if testnet == "true" or testnet == "t":
            print("Updating Bytecash Seeder to testnet settings")
            port = 58444
            network = "testnet7"
            bootstrap = ["testnet-node.bytecash.net"]
            ...
            print("Updating Bytecash Seeder to mainnet settings")
            port = 8444
            network = "mainnet"
            bootstrap = ["node.bytecash.net"]
    ...
    if change_made:
        print("Restart any running Bytecash Seeder services for changes to take effect")
