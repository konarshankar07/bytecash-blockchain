...
from bytecash.consensus.coinbase import create_puzzlehash_for_pk
from bytecash.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_bytecash_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from bytecash.util.bech32m import encode_puzzle_hash
from bytecash.util.config import (
    create_default_bytecash_config,
    initial_config_file,
    load_config,
    save_config,
...
from bytecash.util.ssl_check import (
...
from bytecash.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
from bytecash.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'bytecash keys generate'")
        return None
...
def create_all_ssl(root_path: Path):
    ...
    bytecash_ca_crt, bytecash_ca_key = get_bytecash_ca_crt_key()
    bytecash_ca_crt_path = ca_dir / "bytecash_ca.crt"
    bytecash_ca_key_path = ca_dir / "bytecash_ca.key"
    write_ssl_cert_and_key(bytecash_ca_crt_path, bytecash_ca_crt, bytecash_ca_key_path, bytecash_ca_key)
    ...
    bytecash_ca_crt, bytecash_ca_key = get_bytecash_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, bytecash_ca_crt, bytecash_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                bytecash_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return bytecash_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)


def bytecash_version_number() -> Tuple[str, str, str, str]:
...
def bytecash_minor_release_number():
    res = int(bytecash_version_number()[2])
    print(f"Install release number: {res}")
    return res


def bytecash_full_version_str() -> str:
    major, minor, patch, dev = bytecash_version_number()
...
def bytecash_init(
    ...
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run bytecash init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Bytecash directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run bytecash init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'bytecash keys show --show-mnemonic-seed'")
