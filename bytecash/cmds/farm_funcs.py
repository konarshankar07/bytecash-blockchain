...
from bytecash.cmds.units import units
from bytecash.consensus.block_record import BlockRecord
from bytecash.rpc.farmer_rpc_client import FarmerRpcClient
from bytecash.rpc.full_node_rpc_client import FullNodeRpcClient
from bytecash.rpc.wallet_rpc_client import WalletRpcClient
...
from bytecash.util.misc import format_bytes
from bytecash.util.misc import format_minutes
from bytecash.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total bytecash farmed: {amounts['farmed_amount'] / units['bytecash']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['bytecash']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['bytecash']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'bytecash start wallet' and 'bytecash wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'bytecash wallet show'")
    else:
        print("Note: log into your key using 'bytecash wallet show' to see rewards for each key")
