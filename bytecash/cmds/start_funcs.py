...
from bytecash.cmds.passphrase_funcs import get_current_passphrase
from bytecash.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from bytecash.util.keychain import Keychain, KeyringMaxUnlockAttempts
from bytecash.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    bytecash = sys.argv[0]
    process = subprocess.Popen(f"{bytecash} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the bytecash daemon")
        return None
