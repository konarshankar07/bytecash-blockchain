...
from bytecash.protocols.protocol_state_machine import message_requires_reply
from bytecash.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from bytecash.protocols.shared_protocol import protocol_version
from bytecash.server.introducer_peers import IntroducerPeers
from bytecash.server.outbound_message import Message, NodeType
from bytecash.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from bytecash.util.errors import Err, ProtocolError
from bytecash.util.ints import uint16
from bytecash.util.network import is_localhost, is_in_network
from bytecash.util.ssl_check import verify_ssl_certs_and_keys
...
class ChiaServer:
    def __init__(
        ...
        bytecash_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.bytecash_ca_crt_path, self.bytecash_ca_key_path = bytecash_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.bytecash_ca_crt_path, self.bytecash_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.bytecash_ca_crt_path, self.bytecash_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSChiaConnection] = None
        try:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use bytecash's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.bytecash.net/") as resp:
                    if resp.status == 200:
