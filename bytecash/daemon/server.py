...
from bytecash.cmds.init_funcs import check_keys, bytecash_init
from bytecash.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from bytecash.daemon.keychain_server import KeychainServer, keychain_commands
from bytecash.daemon.windows_signal import kill
from bytecash.plotters.plotters import get_available_plotters
from bytecash.plotting.util import add_plot_directory
from bytecash.server.server import ssl_context_for_root, ssl_context_for_server
from bytecash.ssl.create_ssl import get_mozilla_ca_crt
from bytecash.util.bytecash_logging import initialize_logging
from bytecash.util.config import load_config
from bytecash.util.json_util import dict_to_json_str
from bytecash.util.keychain import (
    Keychain,
...
from bytecash.util.service_groups import validate_service
from bytecash.util.setproctitle import setproctitle
from bytecash.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Bytecash.")
    quit()
...
service_plotter = "bytecash_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "bytecash": "bytecash",
        "bytecash_wallet": "start_wallet",
        "bytecash_full_node": "start_full_node",
        "bytecash_harvester": "start_harvester",
        "bytecash_farmer": "start_farmer",
        "bytecash_introducer": "start_introducer",
        "bytecash_timelord": "start_timelord",
        "bytecash_timelord_launcher": "timelord_launcher",
        "bytecash_full_node_simulator": "start_simulator",
        "bytecash_seeder": "bytecash_seeder",
        "bytecash_seeder_crawler": "bytecash_seeder_crawler",
        "bytecash_seeder_dns": "bytecash_seeder_dns",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["bytecash", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("bytecash daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "bytecash_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in bytecash_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    bytecash_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("bytecash_daemon")
...
def main(argv) -> int:
    from bytecash.util.default_root import DEFAULT_ROOT_PATH
    from bytecash.util.keychain import Keychain
