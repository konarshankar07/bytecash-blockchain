...
from bytecash.cmds.init_funcs import create_all_ssl, create_default_bytecash_config
from bytecash.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from bytecash.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from bytecash.util.errors import Err
from bytecash.full_node.generator import setup_generator_args
from bytecash.full_node.mempool_check_conditions import GENERATOR_MOD
from bytecash.plotting.create_plots import create_plots, PlotKeys
...
from bytecash.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from bytecash.consensus.condition_costs import ConditionCost
...
from bytecash.consensus.deficit import calculate_deficit
from bytecash.consensus.full_block_to_block_record import block_to_block_record
...
from bytecash.consensus.vdf_info_computation import get_signage_point_vdf_info
from bytecash.full_node.signage_point import SignagePoint
from bytecash.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from bytecash.plotting.manager import PlotManager
from bytecash.server.server import ssl_context_for_server
...
from bytecash.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from bytecash.types.blockchain_format.pool_target import PoolTarget
from bytecash.types.blockchain_format.program import INFINITE_COST
from bytecash.types.blockchain_format.proof_of_space import ProofOfSpace
from bytecash.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from bytecash.types.blockchain_format.sized_bytes import bytes32
from bytecash.types.blockchain_format.slots import (
...
from bytecash.types.generator_types import BlockGenerator, CompressorArg
...
from bytecash.util.block_cache import BlockCache
from bytecash.util.condition_tools import ConditionOpcode
from bytecash.util.config import load_config, save_config
from bytecash.util.hash import std_hash
from bytecash.util.ints import uint8, uint16, uint32, uint64, uint128
from bytecash.util.keychain import Keychain, bytes_to_mnemonic
from bytecash.util.merkle_set import MerkleSet
from bytecash.util.prev_transaction_block import get_prev_transaction_block
from bytecash.util.path import mkdir
from bytecash.util.vdf_prover import get_vdf_info_and_proof
from tests.time_out_assert import time_out_assert
from tests.wallet_tools import WalletTool
from bytecash.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_bytecash_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="bytecash-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `bytecash generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("CHIA_ROOT", "~/.bytecash/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
