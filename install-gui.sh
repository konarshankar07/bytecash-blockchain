...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the bytecash python virtual environment."
  echo "Execute '. ./activate' before running."
	exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Bytecash Blockchain GUI can not be installed or run by the root user."
	exit 1
fi
...
echo ""
echo "Bytecash blockchain install-gui.sh completed."
echo ""
echo "Type 'cd bytecash-blockchain-gui' and then 'npm run electron &' to start the GUI."
